...
products.sort(key=lambda product: (product['original_price'] - product['discounted_price']) / product['original_price'], reverse=True)
...
for product in products:
    ...
    if discount_percentage >= 70:
        message = f"{product['product_name']} at R{product['discounted_price']} ({discount_percentage:.2f}% off). {product['product_link']}"
